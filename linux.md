---
layout: large
title: Linux Install Guide
permalink: /linux/
---

# Repair Cafe

Starting March 2026, Galway IT is joining the [Galway Repair Cafe](https://upsew.ie/repair-cafe/) to help people get their unsupported Windows 10 laptops working with Linux (specifically [Linux Mint](https://linuxmint.com/))!

If you'd like to update your laptop to Linux, make sure to book a slot on the Repair Cafe Eventbrite. Otherwise we may not have the time to help you.

You are of course welcome to come along, have a look and try out Linux before commiting to making the change on your own device. Once you are ready to take the plunge, have a read over the steps below to be ready for Linux!



## Getting prepared

Make sure you've done the following before you come along to convert your laptop to Linux:

## Back Up Your Files
Copy everything you want to keep — documents, photos, music, downloads — to an external hard drive or USB stick. **Installing Linux will wipe your drive, so this is essential.** The simplest way is to plug in your external drive and drag and drop your important folders across in File Explorer. Do it twice if you're unsure.

As an extra safety net, check whether you already have files synced to Google Drive, OneDrive or iCloud — many people do without realising it.

Unsure about how to back things up? Feel free to come along and ask us, or visit this link here: [How to back up your files](https://backupyourfiles.neocities.org/).

## Save Your Browser Bookmarks
Export your bookmarks before you arrive so you don't lose them:
- **Chrome / Edge:** Menu → Bookmarks → Export Bookmarks
- **Firefox:** Menu → Bookmarks → Manage Bookmarks → Import and Backup → Export

Copy the exported file to an external drive.

## Back Up Your Passwords
If you use a password manager like Bitwarden, 1Password, or LastPass, your passwords are already safe and you don't need to do anything.

Otherwise, export your saved passwords from your browser:
- **Chrome / Edge:** Settings → Password Manager → click the menu icon → Export
- **Firefox:** Type `about:logins` in the address bar → menu in the top right → Export Logins
- **Safari:** File → Export → Passwords

> ⚠️ The exported file contains all your passwords in plain text — keep it on your backup drive and treat it carefully.

## Note Down Your Wi-Fi Details
You'll need your home Wi-Fi network name and password to get back online after the install. It's usually on a sticker on your router.

## Think About the Software You Use
Make a note of any programs you rely on day-to-day — things like Microsoft Office, Zoom, Spotify, or anything work-related. Bring the list along and we'll help you get set up, or if necessary, we'll help find Linux alternatives on the day.

---

## On the Day, Bring:
- Your laptop
- Your charger / power adapter
- Your backup drive (if you have one!)
- Your notes (Wi-Fi details, software list, passwords)

---

> ⚠️ It's important to note that we're providing this service as volunteers, and we're not in a position to offer dedicated support outside of the events. We'll do what we can to ensure everything 'just works' on the day, and we'll be happy to help where we can at any future Linux events - but ultimately we're volunteers donating our own time and efforts!

If anyone would like to hire us as their full time tech support, however, you're welcome to make us an offer. 😂

Additionally we don't be covering the following at these events. We're happy to offer advice on any of these tasks - but ultimately they're too time consuming!

- Backups (Documents, Photos, Passwords, Browser set-ups, Games, etc.) 
- Hardware Repairs

If the Repair Café times and dates don't suit, or if Linux Mint isn't your speed - feel free to reach out to us at hello@galwayit.org. If there's enough interest, we can take a look at running an Install Party as part of our regular schedule!

Interested in volunteering for a Repair Café or Install Party? Let us know at hello@galwayit.org
---

## More Links
- [End of 10](https://endof10.org/)
- [Right To Repair EU](https://repair.eu/)
- [The Restart Project](https://therestartproject.org/end-of-windows-10-toolkit-for-repair-groups/)
- [New to Linux Mint? Focus on These Three Tools - It's FOSS](https://itsfoss.com/linux-mint-basic-settings/)
- [Linux Mint](https://linuxmint.com/)
- [Linux Mint User Guide](https://linuxmint-user-guide.readthedocs.io/en/latest/)
- [Tech Support Cheat Sheet - xkcd](https://imgs.xkcd.com/comics/tech_support_cheat_sheet.png)
